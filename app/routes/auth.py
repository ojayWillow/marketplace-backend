...
def _ensure_supabase_user(user, phone=None, email=None):
    ...
    Returns:
        ...
        - supabase_user_id: the Supabase UUID to use for session generation
          (may differ from user.supabase_user_id in the conflict case)
        - password_used: password set during creation (or None)
        - conflict_email: real email of the conflicting Supabase account,
          used as sign-in credential when the UUID belongs to another local user
    """
    try:
    ...
    try:
        ...
        if existing:
            ...
            if conflict:
                ...
                # Return existing_id so the caller can still generate a session.
                # conflict_email is the real email on the Supabase account so
                # sign_in_with_password uses the correct credential.
                conflict_email = (
                    conflict.email
                    if conflict.email and not conflict.email.endswith('.kolab.local')
                    else None
                )
...
def _get_supabase_session(user, phone=None, email=None, password=None,
                          conflict_email=None, supabase_user_id_override=None):
    """Generate a Supabase session for a user.

    supabase_user_id_override: use this UUID instead of user.supabase_user_id.
    This is required in the conflict case where user.supabase_user_id is NULL
    in the DB (we deliberately don't overwrite another user's link) but we
    still need to generate a session against the existing Supabase account.

    conflict_email: real email on the conflicting Supabase account. Used as
    the sign-in credential so sign_in_with_password matches the right account.
    """
    supabase_user_id = supabase_user_id_override or user.supabase_user_id
    if not supabase_user_id:
        return None

    # Prefer conflict_email so we sign in against the correct Supabase account.
    # Fall back to the caller-supplied email or the user's own email.
    sign_in_email = conflict_email or email or user.email

    # Never use a .kolab.local placeholder — it doesn't exist in Supabase.
...
def phone_verify():
    ...
    try:
        ...
        supabase_session = _get_supabase_session(
            ...
            supabase_user_id_override=supabase_user_id,
        )
